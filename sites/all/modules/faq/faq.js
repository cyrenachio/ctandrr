  ...
  Drupal.behaviors.initFaqModule = {
    attach: function (context) {
      ...
      $('div.faq-dt-hide-answer:not(.faq-processed)', context).addClass('faq-processed').click(function() {
		...
		var faqopen = $(this).find('span').attr('id');
		console.log(faqopen);
		if($('#'+faqopen).attr('class')=='faq_closed'){
			$('#'+faqopen).removeClass('faq_closed');
			$('#'+faqopen).addClass('faq_open');
		}else{
			$('#'+faqopen).addClass('faq_closed');
			$('#'+faqopen).removeClass('faq_open');
		}
		...
		//$('#'+hash).prepend('<span style=" font-size:20px;cursor: pointer;">-</span>');
        var fx, node = $('#' + hash);
        if (node.length) {
          fx = $('<div></div>')
