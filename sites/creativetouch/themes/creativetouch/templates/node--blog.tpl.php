...
<div id="node-<?php print $node->nid; ?>" class="<?php print $node->type; ?> <?php print $classes; ?> clearfix"<?php print $attributes; ?>>
  <div class="inner-area-left clearfix"<?php print $content_attributes; ?>>
    <?php
	  ...
	  //for image
				if(isset($node->field_blog_image[$node->language][0]['uri'])){
					$style = 'large';
					$filePath = $node->field_blog_image[$node->language][0]['uri'];
				...
				$Description = $node->body[$node->language][0]['safe_value'];
				$Description = strip_tags($Description, '');
				//get alias for URL
				$infoArr = path_load('node/'.$node->vid);
				//print_r($infoArr);
